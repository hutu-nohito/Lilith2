...
public class Spike : Magic_Parameter {
    ...
    IEnumerator Shot()
    {
        Parent.GetComponent<Character_Manager>().Reverse_Magic();//ちょっと魔法を使えなくする
        GameObject bullet;
        ...
        //弾を飛ばす処理
        bullet.transform.position = transform.position;//足元

        yield return new WaitForSeconds(0.1f);//生成してちょっとしたら生えてくる

        bullet.transform.rotation = Quaternion.LookRotation(Vector3.up);//回転させて弾頭を進行方向に向ける
        bullet.GetComponent<Rigidbody>().velocity = (Vector3.up * bullet.GetComponent<Attack_Parameter>().speed);

        yield return new WaitForSeconds(0.15f);//止める
        ...
        //硬直を解除
        Parent.GetComponent<Character_Manager>().Reverse_Magic();
