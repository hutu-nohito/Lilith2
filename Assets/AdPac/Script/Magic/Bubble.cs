...
public class Bubble : Magic_Parameter {

    //跳ねさせるためにボールを床にたたきつける
    ...
    IEnumerator Shot()
    {
        ...
        bullet.GetComponent<Rigidbody>().velocity = ((Parent.GetComponent<MousePoint>().worldPoint - Parent.transform.position).normalized - Vector3.down) * bullet.GetComponent<Attack_Parameter>().speed;
        */
        ...
        {
            bullet.GetComponent<Rigidbody>().velocity = (Parent.transform.TransformDirection(Vector3.forward).normalized - Vector3.down) * bullet.GetComponent<Attack_Parameter>().speed;//キャラの向いてる方向
        }
        else
        {
            bullet.GetComponent<Rigidbody>().velocity = ((Camera.main.ScreenToWorldPoint(new Vector3(Screen.width / 2, Screen.height / 2, 50)) - transform.position).normalized - Vector3.down) * bullet.GetComponent<Attack_Parameter>().speed;//画面の真ん中
        }
        //注目中だったら いらない？
        /*if (pcZ.GetF_Watch())
        {
            //ちょっと上を狙わないと地面に向かってく
            bullet.GetComponent<Rigidbody>().velocity = ((Camera.main.GetComponent<Z_Camera>().Target.transform.position + new Vector3(0, Camera.main.GetComponent<Z_Camera>().Target.transform.localScale.y, 0) - transform.position).normalized - Vector3.down) * bullet.GetComponent<Attack_Parameter>().speed;//敵の方向
        }*/
